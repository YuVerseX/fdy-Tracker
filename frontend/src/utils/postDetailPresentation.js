...
const formatJobSource = (value) => {
  ...
  if (Object.values(labels).includes(normalizedValue)) {
    return normalizedValue
  }
  return labels[normalizedValue] || '其他来源'
}
...
export function getSummarySourceLabel(value) {
  const normalizedValue = normalizeText(value)
  if (!normalizedValue) return ''
  if (Object.prototype.hasOwnProperty.call(SUMMARY_SOURCE_LABELS, normalizedValue)) {
    return SUMMARY_SOURCE_LABELS[normalizedValue]
  }
  return '来源待确认'
}

export function getJobSourcesLabel(values = []) {
  const normalizedValues = Array.isArray(values) ? values : [values]
  return collectUniqueText(normalizedValues.map((value) => formatJobSource(value))).join('、')
}
...
export function buildHeroSummary({ postData = {}, fields = {}, jobItems = [] } = {}) {
  ...
  const completeness = getRecordCompletenessFlags(postData)
  ...
  if (completeness.contentMissing || completeness.summaryMissing || completeness.jobsPending) {
    ...
    if (completeness.contentMissing) {
      return followUps.length > 0
        ? `当前记录仅收录公告标题和基础信息，${followUps.join('，')}。`
        : '当前记录仅收录公告标题和基础信息。'
    }
...
export function buildHeroTags(postData = {}, jobItems = []) {
  ...
  if (completeness.contentAvailable) {
    tags.push({ label: '已收录正文', tone: 'info' })
  }

  if (completeness.contentMissing) {
    tags.push({ label: '正文待补充', tone: 'warning' })
  }

  if (completeness.jobsPending) {
    tags.push({ label: '岗位待整理', tone: 'warning' })
  }
  ...
  return tags.filter((item, index, list) => (
...
export function buildSourceNotes(postData = {}, jobItems = []) {
  ...
  const provenance = postData?.record_provenance || {}
  const completeness = getRecordCompletenessFlags(postData)
  const attachmentCount = provenance?.attachment_count !== null && provenance?.attachment_count !== undefined && provenance?.attachment_count !== ''
    ? Number(provenance.attachment_count)
    : (Array.isArray(postData?.attachments) ? postData.attachments.length : 0)
  const summarySourceLabel = getSummarySourceLabel(provenance?.summary_source)
  const jobSources = Array.isArray(provenance?.job_sources) && provenance.job_sources.length > 0
    ? provenance.job_sources
    : jobItems.map((job) => job?.source).filter(Boolean)
  const jobSourcesLabel = getJobSourcesLabel(jobSources)

  if (provenance?.duplicate_resolution?.resolved_from_duplicate) {
    notes.push('当前详情已自动切换到去重后的主记录，避免同源重复公告分散展示。')
  }

  if (summarySourceLabel) {
    notes.push(`摘要来源：${summarySourceLabel}。`)
  }

  if (jobSourcesLabel) {
    notes.push(`岗位来源：${jobSourcesLabel}。`)
  }

  if (completeness.contentMissing) {
    notes.push('暂未收录正文，可先查看原文链接或附件。')
  }

  if (completeness.jobsPending) {
    notes.push('岗位信息仍在整理中，后续会继续补充。')
  }
