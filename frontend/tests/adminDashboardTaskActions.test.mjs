...
test('getTaskActionDefinitions should distinguish failed retry from successful rerun and incremental run', () => {
  ...
  assert.deepEqual(
    failedActions.map((item) => item.key),
    ['retry']
  )
  assert.deepEqual(
    successActions.map((item) => item.key),
    ['rerun', 'incremental']
  )
