...
test('getAdminRuntimeCopy should expose loading wording for unknown runtime', () => {
  ...
  assert.match(copy.description, /更新|现有概览|能力信息/)
  assert.match(copy.emphasis, /后续同步|状态更新|未获取完整信息/)
  assert.doesNotMatch(copy.description, /稍后刷新后再查看最新结果/)
  assert.doesNotMatch(copy.emphasis, /稍后刷新会显示最新状态/)
})
