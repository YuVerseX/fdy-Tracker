...
### Task 3: 在 ui-ux-pro-max 约束下收口任务卡阶段轨迹与主动作
...
在 `frontend/tests/adminDashboardTaskActions.test.mjs` 中把动作收口到“每状态一个主动作”：
...
test('admin task run card should keep one primary task action button plus detail toggle', () => {
  const source = readSource('views/admin/sections/AdminTaskRunCard.vue')

  assert.match(source, /primaryActionDefinition/)
...
- [ ] **Step 3: 用 ui-ux-pro-max 约束实现阶段轨迹与单一主动作**
...
export function buildTaskRunCardPresentation(run = {}, options = {}) {
  ...
  const primaryActionItems = getTaskActionDefinitions(run).slice(0, 1)
...
最后在 `frontend/src/views/admin/adminDashboardTaskActions.js` 中只保留一个主动作：
...
export const getTaskActionDefinitions = (run = {}) => {
  ...
  if (run?.status === 'cancelled') {
    return INCREMENTAL_ACTION_TASK_TYPES.has(taskType)
      ? [resolveActionDefinition(taskType, 'incremental')].filter(Boolean)
      : [resolveActionDefinition(taskType, 'retry')].filter(Boolean)
  }
...
const primaryActionDefinition = computed(() => cardPresentation.value.actionItems[0] || null)
</script>
...
git commit -m "refactor: 收口任务卡阶段轨迹与主动作层级" -m "主要变更：
- 在 ui-ux-pro-max 约束下新增阶段轨迹组件并按 canonical stage 渲染任务卡
- 将任务卡首屏收口为阶段轨迹、当前结果和单一主动作
